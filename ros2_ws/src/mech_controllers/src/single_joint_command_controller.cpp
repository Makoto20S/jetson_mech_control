...
SingleJointCommandController::SingleJointCommandController(
    std::string motion_interface, std::string target_topic)
    : motion_interface_(std::move(motion_interface)),
      target_topic_(std::move(target_topic)),
      clock_([]() noexcept {
...
void SingleJointCommandController::set_clock_for_testing(MonotonicClock clock) noexcept {
  if (clock) clock_ = std::move(clock);
}

std::uint64_t SingleJointCommandController::target_generation() const noexcept {
...
bool SingleJointCommandController::accept(double target, std::int64_t arrival_nanoseconds,
...
controller_interface::CallbackReturn SingleJointCommandController::on_init() {
  try {
...
controller_interface::CallbackReturn SingleJointCommandController::on_configure(
  ...
  if (joint_name_.empty() || !valid_limits(limits_) || !limiter_.configure(limits_)) {
    return controller_interface::CallbackReturn::ERROR;
  }
...
controller_interface::CallbackReturn SingleJointCommandController::on_activate(
    const rclcpp_lifecycle::State&) {
  if (command_interfaces_.size() != 2U ||
      command_interfaces_[0].get_name() != joint_name_ + "/" + motion_interface_ ||
      command_interfaces_[1].get_name() !=
          joint_name_ + "/" + mech::mech_control_core::kCommandGenerationInterface) {
    return controller_interface::CallbackReturn::ERROR;
  }
  double seed = 0.0;
  if (!initial_command(seed) || !std::isfinite(seed)) {
    return controller_interface::CallbackReturn::ERROR;
  }
  command_ = seed;
  limiter_.clear();
...
controller_interface::CallbackReturn SingleJointCommandController::on_deactivate(
    const rclcpp_lifecycle::State&) {
  active_.store(false, std::memory_order_release);
  activation_epoch_.fetch_add(1U, std::memory_order_acq_rel);
  limiter_.clear();
...
SingleJointCommandController::command_interface_configuration() const {
...
controller_interface::return_type SingleJointCommandController::update(
...
bool SingleJointCommandController::set_target(double target) noexcept {
  const auto activation_epoch =
      activation_epoch_.load(std::memory_order_acquire);
