...
struct BoundedTarget final {
  ...
  // Low-level limiter defaults retain the Foundation 4/6 ms budget. The
  // controllers explicitly configure their upstream 100/106 ms policy;
  // this does not change the separate hardware command lease (ADR-018).
...
//               last valid command is frozen, no fresh target authorization.
...
class SingleJointCommandController : public controller_interface::ControllerInterface {
 public:
 ...
 protected:
  SingleJointCommandController(std::string motion_interface, std::string target_topic);
  [[nodiscard]] const std::string& joint_name() const noexcept { return joint_name_; }
  [[nodiscard]] virtual bool initial_command(double& value) const noexcept = 0;
  [[nodiscard]] virtual bool valid_limits(const BoundedTarget&) const noexcept { return true; }

 private:
  [[nodiscard]] bool accept(double target, std::int64_t arrival_nanoseconds,
                            std::uint64_t activation_epoch) noexcept;

  const std::string motion_interface_;
  const std::string target_topic_;
