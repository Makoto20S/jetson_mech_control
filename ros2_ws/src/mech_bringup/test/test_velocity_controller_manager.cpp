// T8: production velocity plugin + ControllerManager + runtime + FakeTransport.
...
constexpr char kControllerName[] = "motor1_velocity_controller";
...
class VelocityControllerManagerTest : public ::testing::Test {
 protected:
  static void SetUpTestSuite() {
    const char* args[] = {"velocity_manager_test", "--ros-args", "--params-file",
                         MECH_BRINGUP_SOURCE_DIR "/config/motor1_velocity_controllers.yaml"};
    rclcpp::init(4, args);
  }
  ...
  void SetUp() override {
    ...
    controller_ = std::dynamic_pointer_cast<mech::mech_controllers::VelocityCommandController>(
  ...
  void expect_velocity_frame(double velocity) {
    ...
    EXPECT_EQ(((b[1] & 15U) << 8U) | b[2], 819U);  // Kd = 1
    EXPECT_EQ((b[3] << 8U) | b[4], 32767);  // position = 0
    EXPECT_EQ(((b[6] & 15U) << 8U) | b[7], 2047U);  // t_ff = 0
    const auto raw_velocity = (b[5] << 4U) | (b[6] >> 4U);
    const double decoded_velocity = raw_velocity * 80.0 / 4095.0 - 40.0;
    EXPECT_NEAR(decoded_velocity, velocity, 80.0 / 4095.0);
  }
  ...
  std::shared_ptr<mech::mech_controllers::VelocityCommandController> controller_;
  std::int64_t now_nanoseconds_{1000000000000};
  std::int64_t last_feedback_nanoseconds_{0};
};

TEST_F(VelocityControllerManagerTest, LoadsDeploymentAndNeedsExplicitTarget) {
  EXPECT_EQ(manager_->get_parameter("update_rate").as_int(), 500);
  EXPECT_DOUBLE_EQ(controller_->get_node()->get_parameter("maximum").as_double(), 0.5);
  EXPECT_DOUBLE_EQ(controller_->get_node()->get_parameter("max_slew_per_second").as_double(), 0.5);
  EXPECT_EQ(controller_->command_interface_configuration().names,
            (std::vector<std::string>{"motor1_joint/velocity", "motor1_joint/command_generation"}));
  ...
  EXPECT_NEAR(speed.get_value(), 10000.0 / 14.0 / 8.0 * 2.0 *
              mech::mech_protocol_cubemars::kPi / 60.0, 1e-12);
  follow(0.0, 5);
  expect_velocity_frame(0.0);
}

TEST_F(VelocityControllerManagerTest, RampsClampsReversesAndStopsOnWire) {
  establish_feedback();
  activate_controller();
  follow(2.0, 101);
  expect_velocity_frame(0.100);  // 0.5 rad/s^2, read-before-write one-cycle lag.
  follow(2.0, 500);
  expect_velocity_frame(0.5);  // YAML bounds, refreshed identical upstream values.
  follow(-2.0, 1100);
  expect_velocity_frame(-0.5);
  follow(0.0, 600);
  expect_velocity_frame(0.0);
...
TEST_F(VelocityControllerManagerTest, StaleUpstreamStopsFramesWhileManagerKeepsCycling) {
  establish_feedback();
  activate_controller();
  follow(0.3, 400);
  expect_velocity_frame(0.3);
  refresh_target_ = false;
  cycle(60);  // 120 ms > upstream hard deadline and hardware lease.
  expect_velocity_frame(0.3);  // Last real frame is frozen, never synthetic zero.
  cycle(100);
  EXPECT_EQ(transmitted(), 0U);
}

TEST_F(VelocityControllerManagerTest, ReclaimCannotReplayOldVelocity) {
  establish_feedback();
  activate_controller();
  follow(0.3, 400);
  expect_velocity_frame(0.3);
  ...
  refresh_target_ = false;
  (void)last_frame();
  EXPECT_FALSE(controller_->set_target(-0.3));
  cycle(100);
  EXPECT_EQ(transmitted(), 0U);
  activate_controller();
  cycle(100);
  EXPECT_EQ(transmitted(), 0U);
  follow(-0.3, 101);
  expect_velocity_frame(-0.100);
}

TEST_F(VelocityControllerManagerTest, RejectsConflictingControllerWithoutTransmitting) {
  auto competitor = manager_->load_controller(
      "competing_velocity", "mech_controllers/VelocityCommandController");
  ASSERT_NE(competitor, nullptr);
  competitor->get_node()->set_parameter({"joint", kJointName});
  ASSERT_EQ(manager_->configure_controller("competing_velocity"),
            controller_interface::return_type::OK);
  establish_feedback();
  activate_controller();
  EXPECT_EQ(drive_switch({"competing_velocity"}, {}),
