...
- ~~**`ADR-012`'s staged freeze/error actions** — no caller yet to drive that behavior; shipping that plugin without consuming `command_stage()` would leave the watchdog decorative.~~ **Closed 2026-09-06 (stage-3 slice, Position sub-mode):** `mech_bringup`'s `Ak30ForceControlRuntime` is the first production consumer of `command_stage()`. It submits fresh controller writes while Following, freezes the last valid command through Holding (the device keeps executing it; the runtime does not re-submit, so the session's stage clock advances on real time), and fails `read()` on Expired within the ADR-012 3-cycle budget, routing through `CompositeSystem`'s fault-latched ERROR path. Nothing ever resolves a missing command to `0.0`. ~~Torque/Velocity command interfaces remain deferred (they need an interface-shape ADR first).~~ **Closed 2026-09-07 ([ADR-014](../adr/ADR-014-ak30-submode-command-interfaces.md), Proposed):** `CanonicalCommand` now carries position/velocity/effort and a joint's single command interface may name any of the three; the runtime maps only the sub-mode's consumed member (Velocity forces effort = 0 because the wire's effort field is feedforward `t_ff` in every sub-mode), `sub_mode` is an explicit deployment parameter, and the URDF interface name must match it (pinned offline by the deployment-files structure test). ADR-014 requires owner approval before the implementation merges.
