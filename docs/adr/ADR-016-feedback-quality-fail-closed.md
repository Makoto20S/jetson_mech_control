...
2026-09-03 的台架实测是：3 秒内解析出 150 条 `0x2968` 反馈，约 **50 Hz**，即周期约 **20 ms**。

两者相除：在真实电机上，快照在每 20 ms 中约有 14 ms 处于 `Stale`，即大约 **70% 的控制周期**。
...
有效期是可配置的部署参数，设备侧的反馈速率按 L07 §3.1.1.1 也是可配置的（1–2000 Hz）。所以这不是矛盾，是两个参数从未被放在一起校验过。
...
4. **反馈有效期必须与设备回报周期一致。** 部署配置新增设备侧反馈周期，`configure()` 拒绝 `feedback_ttl_nanoseconds` 小于该周期的配置——一个短于回报周期的有效期在定义上永远无法被满足。最终的余量倍数需要实测回报速率的抖动证据后确定；在取得该证据前，实现只强制「有效期 ≥ 回报周期」这一必要条件，并在 ADR 的重审触发中保留该数值待定。
...
**需要但目前不掌握的证据：** motor1 当前实际配置的反馈速率及其抖动。2026-09-03 实测约 50 Hz 是一次 3 秒采样的平均值，不是设备配置的读回值，也不含抖动分布。第 4 条的余量倍数在取得该证据前不能定值。

离线验证通过不构成实机验收；设备运行仍受 ADR-006 Decision 第 7 条逐次授权约束。

## Review triggers / 重审触发

- 取得 motor1 实际配置的反馈速率与抖动证据，需要据此确定第 4 条的余量倍数；
- 出现确实需要按反馈质量做连续降级而非二值判断的控制器，需重审替代方案 D；
- 设备侧回报速率变更（例如为缩短掉线检测延迟而提高到数百 Hz），需按 ADR-006 的带宽模型重算总线负载后再调整有效期；
