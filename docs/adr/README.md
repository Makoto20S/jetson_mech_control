...
| [ADR-018](ADR-018-upstream-target-lifetime.md) | Accepted | [Upstream target lifetime](ADR-018-upstream-target-lifetime.md) | `DemoController` 上游目标采用独立的 100/106 ms 暂定有效期；硬件命令租约保持 4/6 ms；参数名明确区分 target 与 command | 20 Hz 持续刷新、100/106 ms 精确边界、并发 producer 与进程失联回归；Jetson 无设备 nominal/stress 测量仅描述该次运行，不声称硬实时上界 |
