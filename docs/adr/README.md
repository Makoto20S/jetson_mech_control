...
| [ADR-017](ADR-017-command-freshness-generation-interface.md) | Proposed | [Command freshness: optional generation interface](ADR-017-command-freshness-generation-interface.md) | 每关节**始终导出**一个 `command_generation` 命令接口，**是否 claim 由控制器决定**；claim 了走强档（代号不变即不发送、撤销时基线重置，一并关闭「重新激活重放旧目标」），只 claim 运动接口走弱档（维持现行为，缺口登记为已接受风险）；据此修订 ADR-014 Decision 2 | 在实现之前提交，待项目负责人批准。**第一版把代号接口写成强制的，已于 2026-09-14 被否决**——它会使标准 `ros2_control` 控制器无法直接驱动本硬件，与「控制器可自由切换」的立项目标冲突；两档设计出自 `02_architecture_and_interfaces.md` 的既有记录。**连带后果**：两条 DISABLED 出口测试的判据必须从「硬件不能被任意控制器糊弄」弱化为「不能被一个 claim 了代号接口的控制器糊弄」 |
