...
| [ADR-017](ADR-017-command-freshness-generation-interface.md) | Proposed | [Command freshness generation interface](ADR-017-command-freshness-generation-interface.md) | 每关节新增 `command_generation` 命令接口；硬件只在代号**变化**时才认为收到新命令，未变化即不发送；撤销授权时把基线重置为缓冲区当前值，一并关闭「重新激活重放旧目标」；据此修订 ADR-014 Decision 2（每关节恰好一个命令接口 → 一个运动命令接口 + 一个代号接口） | 在实现之前提交，待项目负责人批准。前置已验证：真实 `ControllerManager` 下同一关节双命令接口可被同一控制器 claim，两个名字在同一次 `perform_command_mode_switch` 里到达——但该探针用的是假 `SystemInterface` 而非 `CompositeSystem`，只证明上游允许。**最大代价**：不 claim 并改变代号的控制器从此无法命令电机，标准 `joint_trajectory_controller` 等因此不能直接驱动本硬件 |
