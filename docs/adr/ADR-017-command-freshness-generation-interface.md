...
4. **弱档（控制器只 claim 了运动接口）：维持当前行为，并把缺口明确登记为已接受的风险。** 该关节按 manager 循环节奏继续派发命令，硬件无法区分「控制器刷新了」与「manager 转了一圈」。**这一档的缺口是本 ADR 有意不关闭的**，因为关闭它的唯一已知手段就是强制代号接口，而那与立项需求冲突。
   ...
   **2026-09-18 接管补充（项目负责人批准）：** 完整 switch 通过既有反馈有效性与 claim 检查后，对本次新启动、最终未 claim `command_generation` 的 `position` 接口，用本周期 `read()` 已接受的实测位置初始化命令缓冲。首次 claim 和释放后重新 claim 均适用；控制器下一次写入正常覆盖该值。这样即使标准控制器仅在下一次 `update()` 才写命令，切换周期的硬件首帧仍为实测位置保持，不会消费默认零位或旧目标。该初始化不适用于强档、velocity、effort、仅释放或被拒绝的 switch；不改变 generation 基线、`cancel_pending()`、反馈闸门和弱档静默刷新。它不是新的上游目标，也不保证机械停止。
