...
> 最近收敛：2026-09-06（ADR-012/013 导航补全；OQ-02/03/04 按单电机取证台架实测更新；新增 §7 台架实测证据与 §8 板卡问题）
> 作用：把正式 ADR、现场上下文和仍会阻塞设备接入的问题放在一个短入口中。
> 边界：本文件不复制 ADR 决策正文；台架实测证据见 §7，其范围边界以 [ADR-006](../adr/ADR-006-conditional-can0-deployment.md) Decision 第 7 条为准，不构成最终部署 profile 的验收。
...
| [ADR-006](../adr/ADR-006-conditional-can0-deployment.md) | Proposed | 当前单物理通道及其 transport backend 只是等待设备、能力和总线证据的条件式 profile；Decision 第 7 条定义单电机取证台架边界 |
...
| [ADR-014](../adr/ADR-014-ak30-submode-command-interfaces.md) | Accepted | CompositeSystem 单命令接口形状（position/velocity/effort）与 AK3.0 子模式命令映射；Velocity 子模式 effort=0 由 runtime 强制；2026-09-13 批准，接口形状与子模式的匹配由 `Ak30System::on_init` 在设备 I/O 之前运行期强制 |
...
| 单电机台架 | **2026-09-02～09-05 实测（ADR-006 Decision 7 边界内，owner 定调先验证一台电机）**：motor1 经 Jetson USB-CDC → 通用盒子（⑧→`/dev/ttyACM0`，板 3，固件 4.8.8）完成力控三子模式闭环；证据摘要见 §7 | 台架实测；不构成最终 profile 验收 |
...
| CAN transport | **用户确认（2026-08-23）：电机将接入高擎通用盒子**（7路CAN功率板）的 XT30(2+2) 电源+CAN 通道；Jetson 经 Type-C USB CDC（7×`/dev/ttyACM`）收发原始帧。**台架实测：电机通道 Classic CAN 1 Mbps 兼容**（§7）；两台 HI12 接盒子通道或独立适配器**均为候选，未定**。盒子 nominal 波特率数值仍无书面记载（OQ-08） | 用户确认拓扑意向 + 资料事实 + 台架实测；ADR-006 仍 Proposed，激活仍需全部证据 |
| 断能 | 项目方说明已有遥控继电器断能并曾验证 | 用户提供；G3 前仍需当前书面证据 |
| 项目平台基线 | 目标为 Jetson Orin ARM64 / Ubuntu 22.04 / ROS 2 Humble | 构建基线；FND-004A 已于 2026-08-24 在目标机通过 |
| 实际目标机 | `P3767-0000` Orin NX 16GB 模组 + **HZHY HYAI-311UAV 第三方载板**；2026-08-23 完成迁移（JetPack 6.2 / L4T R36.4.3 / Ubuntu 22.04.5 / 内核 5.15.148-tegra，`nvidia-l4t-*` 已 hold、ROS 2 Humble 已装） | 已确认事实（刷机 + 首启验收 + 加固均完成，见升级教程 §12.0/§14.0） |
...
| OQ-02 | 资料部分解决 + 台架部分实测 | **L07 适用性已由项目负责人 2026-09-01 确认并有驱动板证据**；motor1（第三块通信板、⑧→`/dev/ttyACM0`）已在取证台架上实测：力控命令 `0x0868` 与 `0x29` 周期反馈 @50 Hz 均工作、状态字节 0、三子模式闭环。第二台电机与各台完整固件/配置读取（含单圈模式与 `0x2A` 的 Flash 持久状态）仍未知 | 逐台只读固件/配置记录、第二台实机证据 | codec、claim、lifecycle |
| OQ-03 | 台架部分实测 | `0x29` 的编码器来源 L07 未写明；台架实测（§7）支持其为**稳定绝对读数且外力驱动下实时跟随**，hold 回放时 canonical↔原始换算闭环（-263.758°+330.07°=66.3°≈原始 66.4°），但**单圈/多圈卷绕语义（B14）仍未定论**；`0x2A` 来源仍未证 | 连续低速旋转序列记录 + 供应商帧说明 | `joint/position` 来源 |
| OQ-04 | 台架部分实测 | Kt=0.7382 已 owner 担保并经力矩闭环实测（+0.1 N·m 命令 → effort 回显 0.081–0.111 N·m ≈ `Kt×Iq`）；`gear_ratio=8` 三源一致已 verified；定制版其余参数仍按逐台证据锁定 | `.AppParams`/`.McParams` 与供应商确认（截图 vs 导出差异仍开放：编码器偏移 330.07°/336.28°、Rs、Ls） | 标准 `effort` 映射 |
| OQ-05 | 协议范围解决，性能待测 | **L07 反馈协议范围为 1–2000 Hz，上限不再是协议约束而是总线预算约束（500 Hz 为当前目标）**；台架实测反馈 @50 Hz 稳定；transport latency 和 USB batching 未知 | 配置、离线/注入测试、后续抓包与 30 min 统计 | 有效闭环带宽 |
| OQ-06 | 资料部分解决（2026-08-23） | 失控保护机制已证实存在（上位机应用设置：失控时间/失控刹车电流，见 [06 §4.4.3](06_cubemars_material_review.md)）；motor1 实测配置为失控时间 1000 ms / 刹车电流 0（即**自由滑行**，对空转台架良性）；出厂默认值与能否经 CAN 配置仍未知 | 逐台读取"应用功能"页（G0）；G3 后受控断包试验验证实际行为 | 命令租约与进程崩溃安全 |
| OQ-07 | 未解决 | 两台 HI12 的协议、节点、位速率和输出 profile | PNAME/APP_VER、逐台配置读取和抓包 | HI12 接入方案（盒子通道或独立适配器） |
| OQ-08 | 台架部分实测 | 电机通道的 Classic CAN 1 Mbps 兼容性**已在取证台架上实测排除**（`0x2968` @50 Hz 稳定接收，板端 tx_err=0 后正常 ACK）；盒子 nominal 波特率数值仍无书面记载；HI12（默认 500 kbps）能否上盒子通道仍未知 | 高擎书面答复（§5.2 问题 1/2）+ HI12 通道实测 | ADR-006 状态、HI12 接入方案 |
...
## 7. 单电机取证台架实测证据（2026-09-02～09-05，ADR-006 Decision 7 边界内）

以下结论全部来自空载单电机取证台架（Jetson → USB-CDC 通用盒子 → motor1，接口⑧→`/dev/ttyACM0`），**不构成最终部署 profile 的验收**；每项的证据细节见 [06 §6.3](06_cubemars_material_review.md) 与设计文档 [§5](../development/ak30_force_control_adapter_design.md)：

- **映射参数实测收口**：`gear_ratio=8`（三源一致，owner 批准）、`direction_sign=+1.0`（正向指令→顺时针→反馈位置增加→Iq 正为主）、`Kt=0.7382 N·m/A` 力矩闭环（+0.1 N·m 命令 → effort 回显 0.081–0.111 N·m）。Torque 与 Velocity 子模式 `configure()` 通过且真机闭环。
- **力控三子模式闭环**：Torque（+0.1 N·m，低于 ~0.2 N·m 摩擦地板不转，effort 回显验证）；Velocity（Kd 路径 0.3 rad/s，位移 +0.2°，effort 0.118–0.214 N·m 与摩擦稳态互证）；Position（hold 零位移两遍位置逐位恒定；hold+delta +2°/+5° Kp 误差力矩路径首次直接观测；+30° 全程走 14.3° 停在 Kp=1 摩擦稳态误差——物理预期非故障）。
- **传输链路事实**：盒子 USB-CDC 协议独立实现并验证（`MODE_SET_NUM` 应答固件 4.8.8）；FW 4.8.8 透传回复带 3 字节 CAN ID 前缀（厂商未记载，双格式解码均已实测）；第三块通信板的回复为 14 字节无前缀格式——两种硬件行为并存且都被解码链路兼容。
- **仍开放的台架遗留项**：B14 `0x29` 位置单圈/多圈卷绕语义（需连续低速旋转序列记录）；Torque 子模式的原始 ERPM 速度中止（探针增强项）；探针 `hold`/`hold+delta` 的 provisional 映射在跨卷绕边界命令上的行为未测。
- **物理工程事实（复用价值高）**：空载库仑摩擦地板 ~0.2 N·m；`err_ss ≈ T_friction/Kp` 可预算位置伺服稳态误差；无 Kd 纯力矩空载会匀加速到高速（末速度 = 净力矩×时长/惯量）；带 Kd 的 Velocity 命令有力和速度双重上界，是空载台架最安全的默认测试形状。

## 8. 通信板卡问题（2026-09-05，待 owner 向厂商追问）

前三块通用盒子通信板的处置状态：

- **板 1（旧板）**：曾正常工作至 position 测试时崩溃；复核为 USB 接口芯片僵死（重枚举 `error -71/-110` 后从 lsusb 消失），Jetson USB 栈无辜。已拆下，待质保追问。
- **板 2（新板）**：上电起从未应答任何板级查询；LED 判读（功率板心跳闪、通信板心跳灭）+ 说明书⑰口即通信芯片固件烧录口 → **首选假设：出厂未烧应用固件（或固件损坏）**，未证实。待向厂商：(a) 新板是否预烧应用固件；(b) 索取⑰口烧录包；(c) 板 1 质保。
- **板 3（当前）**：完全正常（固件 4.8.8 应答、50 Hz 反馈流、透传恢复），全部台架实测在此板上完成。
- 厂商文档已知矛盾（B 类清单）：接口⑧在说明书总览/针脚表/接线说明中分别称 CAN7/CAN0/CAN1；实测映射为 ⑧→`ttyACM0`。
