...
- 当前阶段：T8 已合并；T9 实机验收完成且已合并。T10 标准控制器互换（JTC）+ ADR-019 硬件位置包络进行中：PR-1 的包络实现已离线完成，JTC 部署、真实插件及弱档位置接管已完成离线验证并执行实机轨迹；顿挫诊断暂缓，精确跟踪和完整停止仍未验收；其余候选（第二台电机与共总线 / HI12 接入）待定。Foundation 已完成。
...
| T10 标准控制器互换（JTC）+ ADR-019 硬件位置包络 | **进行中：PR-1 包络实现完成（PR-1 离线 399 项测试），[ADR-019](../adr/ADR-019-hardware-position-envelope.md) Proposed；JTC 部署及真实插件离线验证完成（本地 / ASan+UBSan / ARM64 各 412 项通过；ARM64 首次旧激活脚本 DDS 发现超时，原样重跑通过），生命周期修复历史验证各 414 项通过；实机轨迹已执行，顿挫诊断于 2026-09-21 暂缓；精确跟踪、完整停止及最终验收仍待确认** | 硬件层对 Position 命令做失效关闭式包络（绝对区间 + 与反馈的最大偏差），任何控制器（含第三方）都受同一道闸门约束；越界不裁剪、不发送、锁存；`motor1.urdf.xacro` 默认 `[-12, 6] rad` / `0.5 rad`（Kp=1 时名义比例项上限 0.5 N·m，不是总力矩上限） |

T10 后续软件扩展（2026-09-21）：PR #19/#20 已合并；用户授权 Position 同时开放
目标速度和前馈力矩，完成离线验证后提交新 PR，实机实验等用户批准。
[接口设计](../development/position_feedforward_design.md)保留位置单接口兼容，新增固定
组合及整组领取/释放；辅助目标独立限幅检查、默认禁用，Kp/Kd 仍由配置设置。
标准 Humble JTC 使用位置＋速度；完整位置＋速度＋前馈接口须由兼容的单个控制器使用，
本轮不新增完整阻抗控制器。顿挫诊断仍暂缓，不把接口扩展当作顿挫已解决的证据。

T10 当前决定（2026-09-21）：用户暂停顿挫根因排查，后续实际使用中若仍有问题再恢复；
此前授权提交生命周期修复及测试/文档并更新 PR #20；PR #19/#20 随后已合并。
Kp=18/Kd=5、10°轨迹的 4 s/2 s 单程对照各重复两次：加速后正向速度相对波动
由约 0.45–0.46 降至 0.25–0.26，但绝对标准差增加，约 3.214° 周期的电流/电压
分量仍存在。约 26 ms 相对滞后模型已通过跨速度验证，不能据此确定驱动或机械根因。
四次动作均完成，但立即停用后的静止检查失败；各次独立被动后查确认静止、零发送及
端口释放。停止发送约 3 s 后设备遥测才切换模式，完整停止行为仍待单独验证。
暂缓排查不等于顿挫已修复、精度/停止验收通过或 ADR-019 已获批准。
重新排查时先核实标定历史；若设备断电，必须先被动复查临时原点和位置范围。

T10 后续调参（2026-09-18）：用户授权 Kp=4、Kd=1 单次同轨迹测试；候选误差边界
收紧至 0.075 rad，保持名义比例项上限 0.3 N·m。约 2 s 时目标已前进 5.0°，
反馈仅前进 0.7°，误差越界触发 PositionEnvelope 锁存，往返未完成。
随后独立被动采样确认静止、零电机命令及端口释放；精确跟踪仍未通过。

T10 实机更新（2026-09-18）：用户决定不再以激活时的 40 ERPM 阻塞后续测试，
授权执行 +0.1745 rad/4 s、保持 1 s、4 s 返回的 JTC action。动作成功，
STRICT 停用、0.3 s 新鲜静止反馈、故障 0、串口释放通过；实际反馈最大仅
增加约 0.6°，结束较起点高约 0.5°，不代表准确跟随 10°。两次严格零运动
保持仍记失败；该前置条件由用户豁免，不改写为通过。ADR-019 仍 Proposed，
PR #19/#20 等待用户功能验收及合并决定；上述 Kp=4 后续试验已观察到实际误差越界锁存。
