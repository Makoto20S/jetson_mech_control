...
| D3 | 供应商/实验资产 | 主仓库不存供应商 PDF/压缩包/可执行文件/大日志/rosbag；`CubeMars/` 和 `company/` 整体忽略，且必须同时被 `.gitignore` 与 `.dockerignore` 排除（`.dockerignore` 漏项会让 `COPY . /workspace` 把本地资料带进镜像，2026-08-27 实测 1156 个文件）；Git 仅保存来源、版本、哈希和受控 URI；小型自有测试向量逐项审查后可跟踪 | 已确认（2026-08-06） | 项目负责人 |
