...
| [ADR-017](../adr/ADR-017-command-freshness-generation-interface.md) | Proposed | 命令新鲜度两档：每关节始终导出 `command_generation` 命令接口，是否 claim 由控制器决定。强档（已 claim）代号不变即不发送、撤销时基线重置，一并关闭重新激活重放；弱档（只 claim 运动接口）维持现行为，缺口登记为已接受风险以保证标准 ros2_control 控制器可直接驱动本硬件。修订 ADR-014 Decision 2；状态接口形状不变 |
