...
| [ADR-017](../adr/ADR-017-command-freshness-generation-interface.md) | Proposed | 命令新鲜度：每关节新增 `command_generation` 命令接口，硬件只在代号变化时才认为收到新命令，未变化即不发送；撤销时基线重置为缓冲区当前值，同时关闭重新激活重放；修订 ADR-014 Decision 2 为「一个运动命令接口 + 一个代号接口」。状态接口形状不变，ADR-015/016 语义不变 |
