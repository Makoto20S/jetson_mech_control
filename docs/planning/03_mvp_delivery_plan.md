...
**有依据的推断**：后续创建一个私有 GitHub 单仓库，STM32、Jetson/ROS、协议、配置、文档和测试共同版本化。下面仅是目录规划，本轮不创建实现代码。
...
| 当前两电机 MVP | **500 Hz** | 目标 **500 Hz** | 100 Hz actual，必要时 200 Hz | 基础 `0x29` profile、第 6 节指标和单总线预算通过 | 规划决定 |
...
1. **有依据的推断**：由项目负责人评审并批准 ADR-001、002、004、006、009；它们决定后续 Issue 边界。
