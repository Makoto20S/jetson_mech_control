...
| ~~用户确认 L02 V1.0.18 的 CAN 协议和参数适用于当前电机；先做伺服扩展帧~~ **已于 2026-09-01 被项目负责人更正取代**：适用手册是 L07（AK3.0 V3.2.0）。客观佐证是本项目驱动板 `AK54-4810-1C-A2` 只出现在 L07。新顺序为**先力控、后伺服** | 2026-09-01 用户更正；L07 第 30~40 页；[ADR-013](../adr/ADR-013-ak30-protocol-baseline.md) | 用户确认事实 + 规划决定 |
| AKE60-8 力控归一化常数与扭矩系数：KV 80、`Kt = 0.7382 N·m/A`、位置 ±12.56 rad、速度 ±40.0 rad/s、力矩 ±15.0 N·m、Kp 0–500、Kd 0–5；`T = Kt × Iq` 中 T 为**输出端**扭矩 | L07 第 37 页（合并单元格已按原页图确认） | 资料事实 + 项目负责人 2026-09-01 担保适用于定制版 |
| `输出端转速 = ERPM ÷ 极对数 ÷ 减速比` | L07 第 41 页 | 资料事实 |
...
| 电机协议 | **（2026-09-01 更新，[ADR-013](../adr/ADR-013-ak30-protocol-baseline.md)）** L07（AK3.0 V3.2.0）的力控扩展帧（控制模式 ID 8）与伺服扩展帧（模式 0–6、15、16）是当前实现目标，力控优先；适用性由项目负责人确认并有驱动板证据，但每台实际固件/ID 仍未知 | 读取固件版本、CAN 配置并与两套 L07 golden frame 交叉验证（golden vector 须逐条反算，不得照抄手册示例） | 项目负责人，G0 | 部分确认 |
| 输出力矩 | **（2026-09-01 更新）** AKE60-8 就列在适用手册 L07 第 37 页：`Kt = 0.7382 N·m/A`，`T = Kt × Iq` 中 T 为输出端扭矩，力矩范围 ±15.0 N·m。项目负责人已担保适用于本定制版，[ADR-009](../adr/ADR-009-effort-semantic-gate.md) 相应解除「Kt 未知」这一阻塞项。**物理精度、方向与零位链路仍未确认** | 外部计量与台架校准；方向/零位链路仍需供应商说明 | 项目负责人 + A，G3 前 | Kt 已确认，精度待验 |
| 电机失联行为 | 上位机存在"失控保护"配置（失控时间 ms + 失控刹车电流 A）；出厂默认值与启用状态未知，本机导出 `timeout_msec=1000`、`timeout_brake_current=0`，后者是关闭还是零电流刹车未定义。**原引证 L02 第 18 页已随基线切换作废，L07 侧的对应章节需另行核对** | 逐台读取应用设置页参数并截图；低能量台架断包测试验证实际行为 | 项目负责人 + B，G0 读取 / G3 验证 | 资料部分解决 |
...
**规划决定（2026-08-19，已于 2026-09-01 按 [ADR-013](../adr/ADR-013-ak30-protocol-baseline.md) 修订）**：不直接采用任何现成 CubeMars/HighTorque 驱动作为核心。电机协议以 **L07（AK3.0 V3.2.0）** 为首要实现依据，**先实现力控扩展帧（控制模式 ID 8），再实现伺服扩展帧（模式 0–6、15、16）**；两者同为 29 位扩展帧，codec/session 共享 canonical 接口但不共享会混淆位域的打包逻辑。~~原文为「以 L02 V1.0.18 为依据，先伺服 29 位扩展帧、再运控/MIT 11 位标准帧」~~——L02 是 AK2.0 驱动器手册，其覆盖的驱动板与本项目的 `AK54-4810-1C-A2` 不符，该 11 位标准帧 profile 在本项目硬件上不存在。HighTorque FDCAN 示例只作为 USB CDC raw-CAN transport 参考；每个 backend、codec 和 session 均需以离线 golden/negative/boundary 测试和后续逐台证据闭合，设备能力和缩放按准确配置，不继承第三方库中的型号常量。
