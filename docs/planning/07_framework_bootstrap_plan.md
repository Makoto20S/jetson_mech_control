...
| Git | `D:\Work\jetson` 不是有效 Git 仓库；`.git/` 是空目录 | 后续可在当前根执行 `git init`，但必须先建立忽略和资产策略 |
| 供应商资料 | `CubeMars/` 是独立嵌套 Git 仓库 | 主仓库必须忽略它，避免误提交为 gitlink 或复制供应商资产 |
| 实现代码 | 尚不存在 | 不需要兼容旧实现，可按已批准边界干净起步 |
...
计划中的标准命令应在 M0/M1 后由 CI 实际验证，再写入根 README 和 AGENTS.md：

```bash
source /opt/ros/humble/setup.bash
rosdep install --from-paths ros2_ws/src --ignore-src -r -y
colcon build --base-paths ros2_ws/src --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=RelWithDebInfo
colcon test --base-paths ros2_ws/src --event-handlers console_direct+
colcon test-result --verbose
```

这些目前是计划命令，不得在验证前写成“已通过”。
