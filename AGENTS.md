...
发生冲突时按以下顺序处理，并主动修正过期的 memory 或 handoff：
...
- 对任何非平凡新项目任务（包括代码、配置、测试、文档、架构、计划或项目状态变更，以及形成项目结论的调查、决策和验证），必须使用 `project-memory`。开始时恢复上下文；每个新项目任务结束或暂停前，AI 必须自动更新并验证 `memory/STATE.md` 与 `memory/PLAN.md`，只有新增长期确认事实时才更新 `memory/MEMORY.md`。
...
2. 使用 `project-memory`，依次读取 `memory/MEMORY.md`、`memory/STATE.md`、`memory/PLAN.md`。
...
6. 从 `STATE.md` 的 `Immediate Next Action` 或 `PLAN.md` 选择一个明确任务；若用户另有最新指令，以用户指令为准并更新计划。
...
1. 完成与风险相称的测试或检查；
2. 自动更新 `memory/STATE.md` 和 `memory/PLAN.md`；
